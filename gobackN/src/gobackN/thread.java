...
import static gobackN.sender.lost;
import static gobackN.sender.server;
import static gobackN.sender.ports;
import static gobackN.sender.window;
                  ...
                  buf1 = new byte[packetsize];  int flag=1;
                    ...
                    try {   
                     ...
                     if(flag==0){ 
                        ...
                        for(int y=0;y<window;y++){
                            if(a+y<n){
                      msg=String.valueOf(sequence[a+y]);
                        msg=msg.concat("-");
                        msg=msg.concat(new String(result.get(a+y))); 
                          buf = msg.getBytes();
                      DpSend =new DatagramPacket(buf, buf.length, address, clientport);
                       ds.send(DpSend); 
                         System.out.println("Send Data> "+"packet"+sequence[a+y]+"  To port: "+serverport+"  Clientport: "+clientport);
                        }
                        }
                     } 
